...
// user's saved predictions. Used by:
...
// `bracket` is `string` rather than the literal union because the underlying
// DB column is `text`, not a pgEnum, so Drizzle types it as string. Callers
// will pass the relaxed type and we narrow at use sites.
type CascadeMatchup = {
  id: string;
  bracket: string;
  roundIndex: number;
...
// Same byeMap semantics as PredictionExperience uses, exposed for reuse
// in the static /predict bracket overlay.
export function bracketByeSet(ms: CascadeMatchup[]): Set<string> {
  ...
  for (const m of ms) {
    ...
    if (m.roundIndex === 1) {
      if (m.bracket === "losers") {
        if (!aSeated && !bSeated) {
          const willFillEither = ms.some(
            (x) =>
              x.bracket === "main" && x.loserNextMatchupId === m.id
          );
          if (!willFillEither) out.add(m.id);
          // Also a phantom bye when only ONE main loser routes here (the
          // late-add Adam-Erin tail).
      ...
      // Main R1: single-side seated → bye. Both empty shouldn't happen.
